...
if(length(cmd) != 2)
    stop( "Usage: Rscript score.R <predictions> <multiple true labels>" )
...
i <- which( map_int(XX, ncol) == 3 )
if( length(i) < 1 ) stop( "Predictions must be in three-column format." )
if( length(i) > 1 ) stop( "True labels file is in the wrong format" )
Xpred <- nth(XX, i) %>% select( CellID, everything() )
XX = XX[-i]
cmd = cmd[-i]
XX = lapply(1:length(XX), function(ind){ XX[[ind]]$file=cmd[ind]; XX[[ind]]})
Xtrue = do.call(rbind, XX)
